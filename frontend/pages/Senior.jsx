...
const TabsList = ({ children, className = "", activeTab, setActiveTab }) => (
  <div className={`inline-flex h-14 items-center justify-center rounded-lg bg-gray-100 p-1 ${className}`}>
    {React.Children.map(children, child => 
      React.cloneElement(child, { activeTab, setActiveTab })
    )}
  </div>
...
const ThreatCard = ({ title, description, examples, type, severity, fontSize }) => {
  const iconMap = {
    phone: "📞",
    bank: "🏦",
    remote: "💻",
  ...
  const textSizeClass = fontSize === 'xl' ? 'text-xl' : fontSize === 'large' ? 'text-lg' : 'text-base';
  const headingSize = fontSize === 'xl' ? 'text-3xl' : fontSize === 'large' ? 'text-2xl' : 'text-xl';
    ...
    <Card className="p-8 bg-gradient-to-br from-white to-gray-50/50 shadow-lg hover:shadow-xl transition-all duration-300 border-gray-200/60">
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className={`font-bold ${headingSize} text-gray-900 leading-tight mb-3`}>{title}</h3>
            <Badge variant={severityColors[severity]} className="text-base px-4 py-2">
              {severity === 'high' ? 'High Risk ⚠️' : severity === 'medium' ? 'Medium Risk ⚠️' : 'Low Risk ✅'}
            </Badge>
          </div>
        </div>
      ...
      <p className={`text-gray-700 mb-6 ${textSizeClass} leading-relaxed font-medium`}>{description}</p>
      
      <div className="space-y-4">
        <h4 className={`font-semibold ${textSizeClass} text-gray-900 flex items-center gap-3`}>
        ...
        <ul className="space-y-3 ml-4">
          {examples.map((example, index) => (
            <li key={index} className={`${textSizeClass} text-gray-800 flex items-start gap-4 group`}>
              <span className="text-red-500 text-lg mt-1 font-bold group-hover:text-red-600 transition-colors">•</span>
              <span className="leading-relaxed group-hover:text-gray-900 transition-colors font-medium">{example}</span>
            </li>
          ))}
        </ul>
      </div>
...
const SafetyGuide = ({ fontSize }) => {
  const textSizeClass = fontSize === 'xl' ? 'text-xl' : fontSize === 'large' ? 'text-lg' : 'text-base';
  const headingSize = fontSize === 'xl' ? 'text-2xl' : fontSize === 'large' ? 'text-xl' : 'text-lg';
  ...
  const guidelines = [
    {
      ...
      description: "Banks will never ask for your OTP over phone. If someone calls asking for OTP, immediately hang up.",
    ...
    },
    {
      ...
      description: "Never install apps like AnyDesk or TeamViewer if someone on phone asks you to. These give them control of your phone.",
      category: "App Safety"
    },
    {
      id: 3,
      ...
      description: "Never share your account number, debit card number, or PIN with anyone over phone or WhatsApp.",
      category: "Banking"
    },
    {
      id: 4,
      ...
      description: "If someone claims emergency or urgent money need, always verify with family members first.",
    ...
    },
    {
      ...
      description: "If someone claims to be from bank, hang up and call the bank directly using the number on your passbook.",
    ...
    },
    {
      ...
      description: "Scammers can fake caller ID to show bank names. Always verify independently.",
      category: "Phone Safety"
    }
  ];

  const guideText = guidelines.map(g => `${g.title}. ${g.description}`).join(' ');
    ...
    <div className="p-8 bg-white shadow-lg rounded-xl border border-gray-200/60">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h3 className={`font-semibold ${headingSize} text-gray-900`}>सुरक्षा गाइड / Safety Guide</h3>
        </div>
        ...
        {guidelines.map((guide) => (
          ...
          >
            ...
            <div className="flex-1">
              <h4 className={`${textSizeClass} font-bold mb-1 text-gray-900`}>{guide.title}</h4>
              <p className={`text-gray-600 font-medium mb-2`} style={{ fontSize: fontSize === 'xl' ? '18px' : fontSize === 'large' ? '16px' : '14px' }}>
                {guide.subtitle}
              </p>
              <p className={`${textSizeClass} font-medium mb-3 text-gray-800 leading-relaxed`}>{guide.description}</p>
              <span className={`inline-block px-3 py-1 rounded-full bg-green-200 text-green-800 font-medium`} 
                    style={{ fontSize: fontSize === 'xl' ? '16px' : fontSize === 'large' ? '14px' : '12px' }}>
                {guide.category}
              </span>
            </div>
          </div>
      ...
      <div className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className={`font-bold ${textSizeClass} text-blue-900 mb-2`}>Print This Guide</h4>
            <p className={`${textSizeClass} text-blue-800 mb-4`}>Keep this guide near your phone for quick reference</p>
            <Button 
...
const EmergencyContacts = ({ fontSize }) => {
  ...
  const contacts = [
    { name: "Cyber Crime Helpline", number: "1930", description: "Report online fraud" },
    { name: "Police Helpline", number: "100", description: "Emergency police help" },
  ...
  return (
    <Card className="p-8 bg-gradient-to-br from-red-50 to-orange-50 shadow-lg border border-red-200">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className={`font-semibold ${headingSize} text-red-900`}>Emergency Numbers</h3>
      </div>

      <div className="grid gap-4">
        {contacts.map((contact, index) => (
          <div key={index} className="flex justify-between items-center p-4 bg-white rounded-lg shadow-sm border border-red-100">
            <div>
              <div className={`font-bold ${textSizeClass} text-gray-900`}>{contact.name}</div>
              <div className="text-gray-600" style={{ fontSize: fontSize === 'xl' ? '18px' : fontSize === 'large' ? '16px' : '14px' }}>
                {contact.description}
              </div>
            </div>
        ...
        <div className="container mx-auto px-4 py-16 max-w-7xl">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              <Badge variant="secondary" className="w-fit bg-white/10 text-white border-white/20 text-lg px-4 py-2">
                👴👵 For Senior Citizens
              </Badge>
              ...
              <p className="text-xl text-white/90 leading-relaxed">
                Simple, clear guidance to protect yourself from phone frauds, fake bank calls, 
                and online scams. Available in multiple languages with audio support.
              </p>
              <div className="flex flex-col sm:flex-row gap-4">
                <Button size="lg" className=" text-green-600 hover:bg-gray-100 shadow-xl text-xl px-8 py-4">
                  🛡️ Start Learning
                </Button>
                <Button variant="outline" size="lg" className="border-white text-white hover:bg-white/10 border-2 text-xl px-8 py-4">
                  🔊 Audio Guide
                </Button>
              </div>
      ...
      <section className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <Tabs defaultValue="guide" className="space-y-8">
          <div className="text-center space-y-4 mb-8">
            <h2 className="text-4xl font-bold text-gray-900">Choose What You Need</h2>
            <p className="text-gray-600 max-w-3xl mx-auto text-xl leading-relaxed">
              Everything you need to stay safe, presented in simple and clear language
            </p>
          </div>
